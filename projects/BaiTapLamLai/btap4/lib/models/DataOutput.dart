...
class DataOutput {
  ...
  static DataOutput fromJson(Map<String, dynamic> json) {
    ...
    List<OfficesInfo> officesInfoValue =
        json["offices"].map<OfficesInfo>((item) {
      return OfficesInfo.fromJson(item);
    }).toList();

    List<OfficialsInfo> officialsInfoValue =
        json["officials"].map<OfficialsInfo>((item) {
      return OfficialsInfo.fromJson(item);
    }).toList();

    return DataOutput(
        normalizedInput: normalizedInputValue,
        officesInfo: officesInfoValue,
        officialsInfo: officialsInfoValue);
  }
}
